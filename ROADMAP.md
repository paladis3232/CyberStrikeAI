...
- ✅ **Standalone MCP servers** (`mcp-servers/`) — `reverse_shell`: TCP reverse-shell listener exposed as MCP tools (start/stop listener, send command, disconnect, status); `pent_claude_agent`: Claude Agent SDK-based pentest assistant MCP; both run via stdio, compatible with CyberStrikeAI, Cursor, VS Code, Claude Code
- ✅ **MCP auth header** — `auth_header` / `auth_header_value` config fields now parsed and enforced by `mcpHandlerWithAuth` middleware; protects the MCP endpoint on dedicated port 8081
- ✅ **Knowledge indexing config** — new `knowledge.indexing` block controls chunk size, overlap, per-item chunk limit, rate-limit delay, max RPM, and retry behaviour; indexer implements exponential-backoff retries and RPM throttling
