...
- 🧠 **持久化记忆**：跨会话键值存储，工具执行结果自动持久化为记忆条目，即使会话压缩或服务重启也不会丢失
- 🔍 **智能回顾**：每次重要操作前自动检索相似记忆条目与知识库预案，避免重复扫描
- ⏰ **时间感知**：每次 AI 系统提示中自动注入当前日期/时间/时区及会话时长
- 📁 对话分组管理：支持分组创建、置顶、重命名、删除等操作
- 🛡️ 漏洞管理功能：完整的漏洞 CRUD 操作，支持严重程度分级、状态流转、按对话/严重程度/状态过滤，以及统计看板
- 📋 批量任务管理：创建任务队列，批量添加任务，依次顺序执行，支持任务编辑与状态跟踪
- 🎭 角色化测试：13 个预设安全测试角色（渗透测试、CTF、Web 应用扫描等），支持自定义提示词和工具限制
- 🎯 Skills 技能系统：22 个预设安全测试技能（SQL 注入、XSS、API 安全等），可附加到角色或由 AI 按需调用
- 🐳 **Docker 生命周期管理**：通过 `run_docker.sh` 或系统设置 UI 进行部署、更新、启停、监控，支持代理/VPN 配置
...
### 角色化测试
- **预设角色**：系统内置 13 个预设的安全测试角色（渗透测试、CTF、Web 应用扫描、API 安全测试、二进制分析、云安全审计等），位于 `roles/` 目录。
...
### Skills 技能系统
- **预设技能**：系统内置 22 个预设的安全测试技能（SQL 注入、XSS、API 安全、云安全、容器安全等），位于 `skills/` 目录。
...
├── roles/               # 角色配置文件目录（含 13 个预设安全测试角色）
├── skills/              # Skills 目录（含 22 个预设安全测试技能）
├── docs/                # 说明文档（如机器人使用说明）
├── images/              # 文档配图
├── config.yaml          # 运行配置
├── run.sh               # 启动脚本
