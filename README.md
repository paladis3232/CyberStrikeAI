...
- 🤖 AI decision engine with OpenAI-compatible models (GPT, Claude, DeepSeek, Qwen, etc.)
...
CyberStrikeAI ships with 140+ curated tools covering the whole kill chain:

- **Network Scanners** – nmap, masscan, rustscan, arp-scan, nbtscan, autorecon
- **Web & App Scanners** – sqlmap, nikto, dirb, gobuster, feroxbuster, ffuf, httpx, dirsearch, dotdotpwn, wfuzz, x8, jaeles, hakrawler, katana
- **Vulnerability Scanners** – nuclei, wpscan, wafw00f, dalfox, xsser, zap
- **Subdomain Enumeration** – subfinder, amass, findomain, dnsenum, fierce, gau, waybackurls
- **Network Space Search Engines** – fofa_search, zoomeye_search
- **API Security** – graphql-scanner, arjun, api-fuzzer, api-schema-analyzer, burpsuite
...
- **Binary Analysis** – gdb, gdb-peda, radare2, ghidra, objdump, strings, binwalk, angr, checksec, one-gadget, ropper, ropgadget
- **Exploitation** – metasploit, msfvenom, pwntools
- **Password Cracking** – hashcat, john, hashpump, fcrackzip, pdfcrack
- **Forensics** – volatility, volatility3, foremost, steghide, exiftool, stegsolve, zsteg, debix, falco
- **Post-Exploitation** – linpeas, winpeas, mimikatz, bloodhound, impacket, responder, smbmap, netexec
- **CTF Utilities** – hash-identifier, jwt-analyzer, libc-database, pwninit, cyberchef
- **System Helpers** – exec, create-file, delete-file, list-files, modify-file, cat, xxd
...
### Role-Based Testing
- **Predefined roles** – System includes 13 predefined security testing roles (Penetration Testing, CTF, Web App Scanning, API Security Testing, Binary Analysis, Cloud Security Audit, Digital Forensics, Container Security, Post-Exploitation, etc.) in the `roles/` directory.
...
| `tool_run` | Auto-saved tool execution results for deduplication |
| `discovery` | Assets, services, and hosts found during reconnaissance |
| `plan` | Multi-step attack plans and engagement objectives |
