// Dashboard page: fetch running tasks, vulnerability stats, batch tasks, tools & Skills stats and render
...
async function refreshDashboard() {
    ...
    try {
        ...
        // Running tasks: unified count of Agent loop tasks + batch queues "running", keeps top KPI consistent with overview
        ...
        // Batch task queues: count by status (running matches batchRunningCount above)
        if (batchRes && Array.isArray(batchRes.queues)) {
        ...
        // Tool calls: monitor/stats returns { toolName: { totalCalls, successCalls, failedCalls, ... } }
        ...
        // Knowledge: { enabled, total_categories, total_items, ... }
        ...
        if (knowledgeRes && typeof knowledgeRes === 'object') {
            if (knowledgeRes.enabled === false) {
                // Feature disabled: show status label, keep numbers as "-"
                ...
                if (knowledgeStatusEl) {
                    if (items > 0 || categories > 0) {
                        ...
                        knowledgeStatusEl.textContent = (typeof window.t === 'function' ? window.t('dashboard.toConfigure') : 'Not configured');
                    }
                }
            }
        } else {
        ...
        if (skillsRes && typeof skillsRes === 'object') {
            ...
            if (statusEl) {
                if (totalCalls === 0) {
                    statusEl.textContent = (typeof window.t === 'function' ? window.t('dashboard.toUse') : 'Unused');
...
function formatNumber(num) {
    ...
    try {
        ...
        // Fallback: manual thousands separator (safe in all environments)
...
// Top 30 tool call count bar chart colors (30 unique, soft, distinguishable colors)
